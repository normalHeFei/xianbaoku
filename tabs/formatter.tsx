...
const XiaoHongShuFormatter = ({ posts }: { posts: PostDetail[] }) => {
  ...
  const downloadAllImages = () => {
    ...
    Promise.all(allImageUrls.map((item, index) => downloadImage(item.url, `${item.postTitle}-${index + 1}`)))
      .then(() => {
        setIsLoading(false)
        alert('所有图片下载完成')
      })
            ...
            <ul className="list-disc pl-4">
              {allImageUrls.map((item, index) => (
                <li key={index} className="mb-2 break-all">
                  <span className="font-semibold">{item.postTitle}-{index + 1}:</span> {item.url}
                </li>
              ))}
            </ul>
          ) : (
...
const FormatterTab = () => {
  ...
  const fetchPostDetail = async (url: string): Promise<PostDetail | null> => {
    try {
      ...
      const content = doc.querySelector('.article-content')?.textContent || '无内容'
